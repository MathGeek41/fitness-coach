// netlify/functions/analyze-meal.js
...
async function verifyWithUSDA(foodEnglishName, grams) {
  try {
    ...
    return null; // USDA unavailable — continue with Gemini only
  }
}
...
exports.handler = async (event) => {
  ...
  try {
    ...
    // Recalculate totals from merged foods
    ...
    return {
      ...
      body: JSON.stringify({
        ...
        carb:           sum('carb_g'),
        fat:            sum('fat_g'),
        health_score:   gemini.health_score,   // 1–10
        health_reason:  gemini.health_reason,  // Hebrew explanation
        confidence_pct: confidencePct,         // 70–95
        usda_verified:  USDA_KEY ? usdaHits + '/' + foods.length : 'N/A',
        foods                                  // per-item breakdown
      })
    };
  } catch (e) {
